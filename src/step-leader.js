...
const next = (f, i=f.step++) => i ? f.it ? (f.it = f.it.sib) || f._affs && f._affs[(f.step = 1)-1] : f._affs[i] : (f.it = f.next) || f._affs && f._affs[i]
const mark = (f, c, i, ch) => {
  while(i = stack.length) if (!(((f = stack[i-1]).inPath || f.step < 0) && stack.pop())) {
    ...
    if (!(c = next(f))) stack.pop().inPath = !(f.step = 0, f.it = null), path.push(f);
...
const unmark = (f, c=stack.push(f)) => {
  while(f = stack.pop()) if (!(--f._affN || (f.inPath = f.isOrig))){
    while(c = next(f)) stack.push(c); f.step = 0, f.it = f._affs = null;
  }
}
