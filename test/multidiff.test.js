    ...
    taus.forEach(tau => {
      ...
      scaffold.push({
        ...
        result: [
          ...
          {wU: id, dt: -1, state: {n: 0}},  {wR: id, dt: -1, state: {n: 0}}
        ]
      })
      scaffold.push({
        desc: `should immediately coalesce all new state updates without triggering a new update for tau ${rel} 0 nodes`,
        ...
        result: [
          {wA: id, dt: -1, state: null},
          {wU: id, dt: -1, state: {n: 1}}, {wR: id, dt: -1, state: {n: 1}}
        ]
      })
    })
  })
  // TODO: we aren't erasing the below tests yet, since we want this behavior
  //   during willAdd/willUpdate. We'll likely change the above tests in a different commit.
  //   so we can change how well-defined setState (inner diff) behaves.
